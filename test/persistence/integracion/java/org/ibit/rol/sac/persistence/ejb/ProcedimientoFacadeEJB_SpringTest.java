...
import test.unitario.persistence.mock.MockUnidadAdministrativaDelegate;
...
public class ProcedimientoFacadeEJB_SpringTest extends 
//AbstractDependencyInjectionSpringContextTests {
AbstractTransactionalSpringContextTests {

	
	protected static Log log = LogFactory.getLog(ProcedimientoFacadeEJB_SpringTest.class);
...
//		EasyMock.replay(accesoManager);
	...
	public void _test01BuscarProcedimientos() {
		Map<String,String> parametros = new HashMap<String, String>();
		Map<String,String> traduccion = new HashMap<String, String>();
		
		traduccion.put("idioma", "ca");
		List<ProcedimientoLocal> procs = procBean.buscarProcedimientos(parametros, traduccion);
		assertTrue(0<procs.size());
	}

	
	
	public void test04comprobarTodosLosProcedimientosTraducidosCatalan() {
		
		System.setProperty("es.caib.rolsac.idiomaDefault","ca");

		List<UnidadAdministrativa> uas = listarUAs();

		int count=0;

		for(UnidadAdministrativa ua : uas) {

			List<ProcedimientoLocal> procs = procBean.listarProcedimientosUO(ua.getId(), 1);
			_("uaId="+ua.getId());

			for(ProcedimientoLocal proc : procs) {
				Traduccion tra = proc.getTraduccion("ca"); 
				if(null==tra) {
					_(proc.getId());
					_(proc.getFechaActualizacion());
					_(proc.getUnidadAdministrativa());
					count++;
				}
			}
	...
	private  List<UnidadAdministrativa> listarUAs() {
		MockUnidadFacadeEJB uaBean = (MockUnidadFacadeEJB) applicationContext.getBean("uaFacadeEJB");
		return uaBean.listarHijosUA(1L);
	}
	
	
	public void _test02comprobarTodosLosProcedimientosTraducidosCatalan() {
		//System.setProperty("es.caib.rolsac.idiomaDefault","ca");
		List<ProcedimientoLocal> procs = procBean.listarProcedimientosUO(1L, 1);
		_(procs.size());
		assertTrue(0<procs.size());
		int count=0;
		for(ProcedimientoLocal proc : procs) {
	...
	/*
	public void testTMP03obtenerProcedimiento() {
		ProcedimientoLocal proc = procBean.obtenerProcedimiento(103100L);
		Traduccion tra = proc.getTraduccion("ca"); 
	}
	
	public void testTMP04BorrarProcedimiento() {
		procBean.borrarProcedimiento(103100L);
