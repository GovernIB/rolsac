...
@Path( "/"+ Constantes.ENTIDAD_TIPO ) 
@Api( value = "/"+ Constantes.ENTIDAD_TIPO,   tags = Constantes.ENTIDAD_TIPO  )
public class TipusService {
	...
	@Path("/")
	@ApiOperation( 
	    value = "Lista de tipos de normativas",
	    notes = "Lista todos los tipos de normativas disponibles"
	)
	@ApiResponses(value = { 
			@ApiResponse(code = 200, message =  Constantes.MSJ_200_GENERICO, response = RespuestaTipus.class),
			@ApiResponse(code = 400, message = Constantes.MSJ_400_GENERICO, response = RespuestaError.class)
		   })
	
	public RespuestaTipus llistar(	
	...
	@Path("/{codigo}")
	@ApiOperation( 
	    value = "Obtiene un tipo de normativa",
	    notes = "Obtiene el tipo de normativa con el id(código) indicado"
	)
	@ApiResponses(value = { 
			 @ApiResponse(code = 200, message =  Constantes.MSJ_200_GENERICO, response = RespuestaTipus.class),
			@ApiResponse(code = 400, message = Constantes.MSJ_400_GENERICO, response = RespuestaError.class)
	...
	public RespuestaTipus  getTipo(  
			@ApiParam( value = "Codigo de idioma", required = false ) @DefaultValue(Constantes.IDIOMA_DEFECTO) @FormParam("lang") final String lang,
			@ApiParam( value = "Código de tipo", required = true ) @PathParam( "codigo") final  String codigo
	 ...
	 private RespuestaTipus getRespuesta(FiltroGenerico fg) throws DelegateException {		
		 es.caib.rolsac.utils.ResultadoBusqueda resultadoBusqueda = DelegateUtil.getTipoNormativaDelegate().consultaTipo(fg);
		
		List <Tipus> lista = new ArrayList <Tipus>();
			
		for (Tipo nodo : Utiles.castList(Tipo.class, resultadoBusqueda.getListaResultados())) {
			Tipus elemento = new Tipus(nodo,null,fg.getLang(),true);
			lista.add(elemento);
		}
		
		RespuestaTipus r = new RespuestaTipus(Response.Status.OK.getStatusCode()+"", Constantes.mensaje200(lista.size()) , new Integer(resultadoBusqueda.getTotalResultados()), lista);
		return r;
	}		
