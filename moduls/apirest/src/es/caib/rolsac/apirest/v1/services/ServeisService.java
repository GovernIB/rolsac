...
public class ServeisService {
	...
	@Path("/")
	@ApiOperation( 
	    value = "Lista los servicios",
	    notes = "Lista los servicios disponibles en funcion de los filtros"
	)
	@ApiResponses(value = { 
	...
	@Path("/{codigo}")
	@ApiOperation( 
	    value = "Obtiene un servicio",
	    notes = "Obtiene el servicio con el código indicado"
	)
	@ApiResponses(value = { 
			@ApiResponse(code = 200, message =  Constantes.MSJ_200_GENERICO, response = RespuestaServeis.class),
			@ApiResponse(code = 400, message = Constantes.MSJ_400_GENERICO, response = RespuestaError.class)
    ...
    private RespuestaServeis getRespuesta(FiltroGenerico filtro) throws DelegateException {		
    	es.caib.rolsac.utils.ResultadoBusqueda resultadoBusqueda = DelegateUtil.getServicioDelegate().consultaServicios(filtro);	
		List <Serveis> lista = new ArrayList <Serveis>();
			
		for (Servicio nodo : Utiles.castList(Servicio.class, resultadoBusqueda.getListaResultados())) {
			Serveis elemento = new Serveis(nodo,null,filtro.getLang(),true);
			lista.add(elemento);
		}
		
		RespuestaServeis r = new RespuestaServeis(Response.Status.OK.getStatusCode()+"", Constantes.mensaje200(lista.size()) , new Integer(resultadoBusqueda.getTotalResultados()), lista);
		return r;
	}
