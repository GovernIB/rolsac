...
public abstract class UARemotaFacadeEJB extends HibernateEJB {
    ...
    public Long grabarUARemota(final String idRemoto, UnidadAdministrativaRemota uaRemota, Long idPadre , String ceTratamiento ,String[] ceMaterias) {
        ...
        try {
            ...
            return grabarUARemota(session, uaRemota, idPadre, ceTratamiento, ceMaterias);
     ...
     */
    @SuppressWarnings("unchecked")
    public Long grabarUARemota(UnidadAdministrativaRemota uaRemota, Long idPadre , String ceTratamiento ,String[] ceMaterias) {
        final Session session = getSession();
        try {
            ...
            if (uaRemota.getAdministracionRemota() != null){
                adminRemota = (AdministracionRemota) session.load(
                        AdministracionRemota.class, uaRemota
     ...
     */
    @SuppressWarnings("unchecked")
    private Long grabarUARemota(final Session session, UnidadAdministrativaRemota uaRemota, Long idPadre , String ceTratamiento ,String[] ceMaterias) throws HibernateException {
        ...
        Tratamiento tratamiento = RemotoUtils.recogerTratamientoCE(session, ceTratamiento);
        if(tratamiento!=null){
        ...
        if(uaRemota.getId()==null){
            if(uaPadre==null){
                ...
                //Si no lo es la a�ado a su padre
            ...
            if(uaRemota.getUnidadesMaterias()==null){
                ...
                for (UnidadMateria uniMat : (Set<UnidadMateria>)uaRemota.getUnidadesMaterias()) {
                    ...
                    for (Materia materia : materias) {
                        if(relacionado = materia.getId().equals(uniMat.getMateria().getId())){
    ...
    /**
     * Obtiene una UnidadAdministrativa Remota segun su idExterno
     * 
     * @ejb.interface-method
     * @ejb.permission unchecked="true"
     */
    ...
    /**
     * Obtiene  Unidad Administrativa Remota segun su id
     * @ejb.interface-method
     * @ejb.permission unchecked="true"
     */
    ...
    /**
     * Obtiene una AdministracionRemota segun su idRemoto
     * 
     * @ejb.interface-method
     * @ejb.permission unchecked="true"
     */
    ...
    /**
     * Lista las unidades Administrativas Remotas por Ambito(Autonomico, Insular)(PORMAD)
     * @ejb.interface-method
     * @ejb.permission unchecked="true"
     */
    public List<UnidadAdministrativaRemota> listarUARemotasPorAmbito(Long ambito) {
        Session session = getSession();
        try {
            Query query = session.createQuery("from UnidadAdministrativaRemota as ua where"+
                                              " ua.validacion = :validacion and " +
                                              " ua.espacioTerrit.nivel = :ambito");
	...
	public void borrarUnidadAdministrativaRaizRemota(Long id) {
		...
		try {
			...
			for (Iterator iter = ua.getEdificios().iterator(); iter.hasNext();) {
	 ...
	 */
	 public void borrarUnidadAdministrativaRemota(Long id) throws Exception {
		...
		try {
			...
			if (ua.isRaiz()) {
				throw new SecurityException(
						"No puede eliminar una unidad raiz.");
			} else {
				if (!getAccesoManager().tieneAccesoUnidad(
						ua.getPadre().getId(), true)) {
					throw new SecurityException(
							"No tiene acceso al padre de la unidad");
				}
			}
			addOperacion(session, ua, Auditoria.BORRAR);
			Historico historico = getHistorico(session, ua);
	 ...
	 /**
	    ...
	    /**
	     * Obtiene un Edificio Remoto apartir de su id externo y su id de la UARemota
	     * @ejb.interface-method
	     * @ejb.permission unchecked="true"
	     */
	    public EdificioRemoto obtenerEdificioRemoto(Long idExterno,Long idUaRemota) {
	        ...
	        try {
				...
				Query query = session.createQuery("from EdificioRemoto as er where er.idExterno="+idExterno+" and er.administracionRemota.id="+idUaRemota);
	    ...
	    /**
	     * Crea o actualiza un Edificio en la importacion de una UA. Se duplica paro no tener roles.
		 * @ejb.interface-method
		 * @ejb.permission unchecked="true"
	     */
	    ...
	    public List<Edificio> obtenerEdificiosUA(Long ua) {
	        Session session = getSession();
	        try {
				Query query = session.createQuery("select elements(ua.edificios) from UnidadAdministrativa as ua where ua.id="+ua);
		...
		/**
		 * Borra un Edifico Remoto segun su idExterno
		 * 
		 * @ejb.interface-method
		 * @ejb.permission unchecked="true"
		 */
		public void borrarEdificioRemoto(String idRemoto ,Long idExt) {
			...
			try {
                ...
                EdificioRemoto edificioRemoto = RemotoUtils.recogerEdificio(session,idExt,admin.getId());
                for (Iterator iterator = edificioRemoto.getUnidadesAdministrativas().iterator(); iterator.hasNext();) {
                    UnidadAdministrativa ua = (UnidadAdministrativa) iterator.next();                    
