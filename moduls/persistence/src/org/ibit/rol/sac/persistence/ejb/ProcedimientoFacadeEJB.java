	...
	public ProcedimientoLocal obtenerProcedimiento(Long id) throws DelegateException {
		...
		try {
			...
			if (visible(procedimiento)) {
				...
				Query query = session.createQuery("select t from Tramite t, t.traducciones as trad where t.procedimiento.id = :idProcedimiento and index(trad) = :idioma ");
				...
				for (Tramite t : tramites) {
					...
					for ( DocumentTramit dt : t.getDocsInformatius() ) {
						...
						Iterator<String> i = idiomes.iterator();
						while ( i.hasNext() ) {
				...
				procedimiento.setTramites(tramites);
