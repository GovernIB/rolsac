...
public abstract class CatalegDocumentsFacadeEJB extends HibernateEJB
{
	...
	private String populateQuery(Map parametros, Map traduccion, List params)
	{
	 ...
	 /**
	 * Consulta los catalogos de documentos en funcion del filtro generico
	 * 
	 * @ejb.interface-method
     * @ejb.permission unchecked="true"
	 */
	public ResultadoBusqueda consultaCatalegDocuments(FiltroGenerico filtro){
		...
		StringBuilder select = new StringBuilder("SELECT cd ");
		StringBuilder selectCount = new StringBuilder("SELECT count(cd) ");
		StringBuilder from = new StringBuilder(" FROM CatalegDocuments as cd, cd.traducciones as trad ") ;
