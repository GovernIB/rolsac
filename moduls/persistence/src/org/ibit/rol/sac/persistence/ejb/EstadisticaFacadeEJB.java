...
public abstract class EstadisticaFacadeEJB extends HibernateEJB {
	...
	/**
	 *  @deprecated No se usa
	 * Normativas m�s visitadas en un periodo
	 * @ejb.interface-method
	 * @ejb.permission unchecked="true"
	 */
	 ...
	 * @param fechaFin	Indica la fecha final del resumen.
	 * 
	 * @return Devuelve <code>List</code> de historicos de procedimientos.
	 */
	public List listarProcedimientosMasVisitados(Long idUnidad, Date fechaInicio, Date fechaFin) {
		...
		try {
			...
			while ( iter.hasNext() ) {
				...
				Long idHistorico = (Long) iter.next();
				HistoricoProcedimiento historico = (HistoricoProcedimiento) session.load( HistoricoProcedimiento.class, idHistorico );
	 ...
	 * @param fechaFin	Indica la fecha final del resumen.
	 * 
	 * @return Devuelve <code>List</code> de histórico de fichas.
	 */
	public List listarFichasMasVisitadas(Long idUnidad, Date fechaInicio, Date fechaFin) {
		...
		try {
			...
			Iterator iteradorFichaUA = unidad.getFichasUA().iterator(); 
			while ( iteradorFichaUA.hasNext() ) {
				
				FichaUA fichaUa = (FichaUA) iteradorFichaUA.next();
			...
			Iterator iteradorResultado = result.iterator();
			while ( iteradorResultado.hasNext() ) {
				
				Long idHistorico = (Long) iteradorResultado.next();
				HistoricoFicha historico = (HistoricoFicha) session.load( HistoricoFicha.class, idHistorico );
	 ...
	 *
	 * @param idMat	Identificador de materia.
	 */
	public void grabarEstadisticaFichaPorMateria(Long idFicha, Long idMat) {
		
		Session session = getSession();
		
		try {
			
			if ( idFicha != null  &&  idFicha.longValue() != 0  &&  idMat != null  &&  idMat.longValue() != 0 ) {
	 ...
	 * @param idUA	Identificador de una unidad administrativa.
	 */
	public void grabarEstadisticaFichaPorUA(Long idFicha, Long idUA) {

		Session session = getSession();

		try {

			if ( idFicha != null  &&  idFicha.longValue() != 0  &&  idUA != null  &&  idUA.longValue() != 0 ) {
