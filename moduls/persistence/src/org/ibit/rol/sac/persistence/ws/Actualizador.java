...
final public class Actualizador {
	...
	static Thread actualizador;  
	static VentanillaUnica vuds;
	
	//u92770[enric] getters and setters
	...
	public static Thread getActualizador() {return actualizador;}
	public static void setActualizador(Thread actualizador) {Actualizador.actualizador = actualizador;}
	...
	public boolean calActualizar(Destinatario dest, Object elem) {
		...
		//si es tram VUDS y dest es vuds -> true
		//si no es tram y dest no es vuds -> true
		//altres casos -> false

    	String value = System.getProperty("es.indra.caib.rolsac.oficina");
        if ((value == null) || value.equals("N")) {
        	//si indra no diu res 
        	if(null!=dest.getNombre() && elem instanceof Tramite) {
        		// es un tramit: comprovam si ̩s VUDS o no.
        		if ("VUDS".equals(dest.getNombre())) {
        			...
        			// si no es un tramit VUDS sempre cal actualitzar
        			return true;
        		}
        	} else {
        		if (null!=dest.getNombre() && !"VUDS".equals(dest.getNombre()) && !(elem instanceof Tramite)) {
        			//si dest no es vuds i no es un tramite
	        		return true;
        		}
        	}
        }
	 ...
	 *
	 */
	private class ActualizadorThread extends Thread {
		...
		public void run() {
			...
			if (actualizar instanceof Ficha) {
				...
				if(params.length == 1){
					if (borrar) {
						borrarTramitePMA(tramit);
					} else {
						actualizarTramitePMA(tramit);
					}
		...
		private void actualizarTramite(final Tramite tramit) {
			log.info("Actualizando un Tramite");
			
			//Y voy destinatario a destinatario mandando la actualizacion
			for (final Destinatario destinatario : destinatarios) {
				try{
					if(calActualizar(destinatario, tramit)) {
						log.info("Al Destinatario: "+destinatario.getNombre());
						if(null==Actualizador.vuds) Actualizador.vuds=new VentanillaUnica();
						if(null!=destinatario.getEndpoint()) vuds.enviarTramit(destinatario.getEndpoint(), tramit,VentanillaUnica.CASTELLA); 
						destinatarisOK.add(destinatario.getNombre());
					}
				} catch (WSInvocatorException e) {
					log.error(e);
					//Si falla mando un Email informando del fallo al destinatario
		...
		/**
		 * Actualiza un {@link Tramite} del portal multiadministratiu
		 * @param edif
		 */
		private void actualizarTramitePMA(final Tramite tram){
			log.info("Actualizando un TramitePMA");
			
			//La transformo en transferible
			final TramiteTransferible tramT = TramiteTransferible.generar(tram);
			String value = System.getProperty("es.indra.caib.rolsac.oficina");
			//Y voy destinatario a destinatario mandando la actualizacion
			for (final Destinatario destinatario : destinatarios) {
				try{
		 ...
		 * @param edif
		 */
		private void borrarTramitePMA(final Tramite tram){
			String value = System.getProperty("es.indra.caib.rolsac.oficina");
			for (final Destinatario destinatario : destinatarios) {
				try{
					if ((value == null) || value.equals("N")) {		
