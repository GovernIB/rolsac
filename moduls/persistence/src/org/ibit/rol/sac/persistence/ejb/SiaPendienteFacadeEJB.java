 ...
 *
 * @ejb.bean name="sac/persistence/SiaPendienteFacade"
...
public abstract class SiaPendienteFacadeEJB extends HibernateEJB {
	 ...
	 * @ejb.permission unchecked="true"
	 */
	public void enviarTodos(final SiaJob siaJob) throws DelegateException {
	 ...
	 * @return
	 */
	private EstadoProcesoSIA enviarTodosProcedimientos(final List<Long> idProcedimientos, final SiaJob siaJob,
			final EstadoProcesoSIA estadoProceso) {
		...
		try {
			...
			for (final Long idProcedimiento : idProcedimientos) {
				...
				try {
					...
					final ProcedimientoLocal procedimiento = procDelegate.obtenerProcedimientoParaSolr(idProcedimiento,
							null);
					...
					if (siaEnviableResultado.isNotificiarSIA()) {
						...
						if (siaCumpleDatos.isCumpleDatos()) {
							try {
								...
								siaResultado = new SiaResultado(SiaResultado.RESULTADO_ERROR,
							...
							DelegateUtil.getSiaPendienteProcesoDelegate().generarSiaPendiente(siaPendiente,
									procedimiento, null);
							siaResultado = new SiaResultado(SiaResultado.RESULTADO_NO_CUMPLE_DATOS,
									siaCumpleDatos.getRespuesta());
						}
					} else {
						siaResultado = new SiaResultado(SiaResultado.RESULTADO_NO_ENVIABLE,
								siaEnviableResultado.getRespuesta());
					}
					...
					log.error("Error no controlado enviando procedimiento (id:" + idProcedimiento + "): "
			...
			actualizaEstadoProceso(null,
					new SiaResultado(SiaResultado.RESULTADO_ERROR, ExceptionUtils.getStackTrace(exception)),
					estadoProceso, "procedimiento");
		}
	 ...
	 * @return
	 */
	private EstadoProcesoSIA enviarTodosServicios(final List<Long> idServicios, final SiaJob siaJob,
			final EstadoProcesoSIA estadoProceso) {
		...
		try {
			...
			for (final Long idServicio : idServicios) {
				...
				try {
					...
					final Servicio servicio = servicioDelegate.obtenerServicioParaSolr(idServicio, null);
					...
					if (siaEnviableResultado.isNotificiarSIA()) {
						...
						if (siaCumpleDatos.isCumpleDatos()) {
							...
							siaPendiente.setMensaje(siaCumpleDatos.getRespuesta());
							DelegateUtil.getSiaPendienteProcesoDelegate().generarSiaPendiente(siaPendiente, null,
									servicio);
							siaResultado = new SiaResultado(SiaResultado.RESULTADO_NO_CUMPLE_DATOS,
									siaCumpleDatos.getRespuesta());
						}
					...
					log.error(
							"Error no controlado enviando servicio (id:" + idServicio + "): " + exception.getMessage(),
							exception);
				}
			...
			actualizaEstadoProceso(null,
					new SiaResultado(SiaResultado.RESULTADO_ERROR, ExceptionUtils.getStackTrace(exception)),
					estadoProceso, "servicio");
		}
		return estadoProceso;
	}

	private SiaResultado enviarProcedimiento(final ProcedimientoLocal proc) throws Exception {
		...
		try {
			...
			if (ex.getMessage() != null && ex.getMessage().contains("0167:")) {
				...
				throw new Exception("Error enviando a SIA el procedimiento " + proc.getId() + ": " + ex.getMessage(),
						ex);
			}
		}
	 ...
	 * @ejb.permission unchecked="true"
	 */
	public SiaResultado enviarProcedimientoNoActivo(final ProcedimientoLocal proc) throws DelegateException {
	...
	private SiaResultado enviarServicio(final Servicio servicio) throws Exception {
		...
		try {
			...
			if (ex.getMessage() != null && ex.getMessage().contains("0167:")) {
				...
				throw new Exception("Error enviando a SIA el servicio " + servicio.getId() + ": " + ex.getMessage(),
						ex);
			}
		}
	 ...
	 * @ejb.permission unchecked="true"
	 */
	public SiaResultado enviarServicioNoActivo(final Servicio servicio) throws DelegateException {
	...
	private void actualizaEstadoProceso(final Long id, final SiaResultado siaResultado,
			final EstadoProcesoSIA estadoProceso, final String literalTabla) {
		...
		if (siaResultado == null || estadoProceso == null) {
			...
			estadoProceso.addLineaDetalle("   ---- " + literalTabla + ": " + id
					+ " s'hauria d'enviar però falten dades: " + siaResultado.getMensaje() + "<br />");
		} else if (siaResultado.isNoEnviable()) {
			estadoProceso.addNoEnviable();
			estadoProceso.addLineaDetalle("   ---- " + literalTabla + ": " + id
					+ " no és visible i no s'ha d'enviar a SIA: " + siaResultado.getMensaje() + "<br />");
	...
	private void guardarEstadoProcesoSIA(final SiaJob siaJob, final EstadoProcesoSIA estadoProceso) {

		final int correctos = estadoProceso.getCorrectos();
		final int incorrectos = estadoProceso.getIncorrectos();
		// int nulos = estadoProceso.getNulos();

		final int noCumpleDatos = estadoProceso.getNoCumpleDatos();
		final int noEnviable = estadoProceso.getNoEnviable();
		final int total = estadoProceso.getTotal();

		final SimpleDateFormat formato = new SimpleDateFormat("H:mm");

		try {

			// Si no esta finalizado, actualizamos cada 50
			if (estadoProceso.getFechaFin() != null
					|| (correctos + incorrectos + noCumpleDatos + noEnviable) % 50 == 0) {
				final StringBuffer resultadoBeta = new StringBuffer();
				resultadoBeta.append("Iniciat a les " + formato.format(estadoProceso.getFechaInicio()) + "<br />");
				if (total != -1) {
					resultadoBeta.append("S'han intentat enviar " + (correctos + incorrectos)
							+ " procediments d'un total de " + total + " procediments disponibles: <br />");
				} else {
					resultadoBeta
							.append("S'han intentat enviar  " + (correctos + incorrectos) + " procediments : <br />");
				}
				...
				if (estadoProceso.getFechaFin() == null) {
					resultadoBeta.append(
							"Estat: " + Math.abs((correctos + incorrectos + noEnviable + noCumpleDatos) * 100 / total)
									+ "%  <br />");
				} else {
	...
	/**
	 * Revisa los procedimientos que podrian estar en SIA y les envia un cambio si
	 * es necesario.
	 *
	 * @throws Exception
	 * @ejb.interface-method
	 * @ejb.permission unchecked="true"
	 */
	public void revisarProcedimientosPorTiempo(final SiaJob siaJob) {
		...
		try {
			final List<Long> listProcedimientos = procDelegate.getProcedimientosEstadoSIAAlterado();
			if (listProcedimientos != null) {
				for (final Long idProcedimiento : listProcedimientos) {
			...
			final List<Long> listServicios = servDelegate.getServiciosEstadoSIAAlterado();
			if (listServicios != null) {
				for (final Long idServicio : listServicios) {
			...
			log.error(e);
			final StringBuffer resultadoDescripcionBreve = new StringBuffer();
			resultadoDescripcionBreve
					.append(" Error obteniendo los procedimientos y servicios caducados. Mensaje:" + e.getMessage());
			...
			try {
				siaPendienteDelegate.actualizarSiaJob(siaJob);
			} catch (final DelegateException e2) {
				log.error(e2);
			}
			return;
		}
	 ...
	 * @throws DelegateException
	 */
	public void enviarPendientes(final SiaJob siaJob) {
		...
		// Inicializamos vble para establecer estado proceso
		final EstadoProcesoSIA estadoProceso = new EstadoProcesoSIA();
		estadoProceso.setFechaInicio(new Date());
		...
		try {
			...
			for (final SiaPendiente siaPendiente : siaPendientes) {
				...
				if (SiaUtils.SIAPENDIENTE_TIPO_PROCEDIMIENTO.equals(siaPendiente.getTipo())) {
					try {
						...
						resultadoPendiente = new ResultadoSiaPendiente(false,
								"Error no controlado para elemento pendiente " + siaPendiente.getId() + ": "
										+ ExceptionUtils.getStackTrace(exception));
						// Actualiza estado proceso
						actualizaEstadoProceso(null,
								new SiaResultado(SiaResultado.RESULTADO_ERROR,
										"Error no controlado para elemento pendiente " + siaPendiente.getId() + ": "
												+ ExceptionUtils.getStackTrace(exception)),
								estadoProceso, "procedimiento");
					}
					...
					resultadoPendiente = new ResultadoSiaPendiente(false, "Error no controlado para elemento pendiente "
							+ siaPendiente.getId() + ": No se tiene conocimiento del Siapendiente.TIPO");
				}
				...
				if (resultadoPendiente.isCorrecto()) {
					...
					final int dias = calcularDias(siaPendiente);
	...
	/**
	 * Enviar SIA de tipo de procedimiento. Antes había tipo Normativa y tipoUA pero
	 * se ha simplificado y sólo hay uno.
	 *
	 * @param estadoProceso
	 * @param siaPendiente
	 * @return
	 * @throws Exception
	 */
	private ResultadoSiaPendiente enviarPendienteTipoProcedimiento(final EstadoProcesoSIA estadoProceso,
			final SiaPendiente siaPendiente) throws Exception {
		...
		if (siaPendiente.getExiste().compareTo(SiaUtils.SIAPENDIENTE_PROCEDIMIENTO_BORRADO) == 0) {
			...
			final SiaResultado siaResultado = borradoProcedimiento(siaPendiente.getIdElemento(),
					siaPendiente.getIdSia().toString(), siaPendiente.getSiaUA());
			...
			final ProcedimientoLocal procedimiento = procDelegate
					.obtenerProcedimientoParaSolr(siaPendiente.getIdElemento(), null);
	 ...
	 * @throws Exception
	 */
	private ResultadoSiaPendiente enviarPendienteTipoServicio(final EstadoProcesoSIA estadoProceso,
			final SiaPendiente siaPendiente) throws Exception {
		...
		if (siaPendiente.getExiste().compareTo(SiaUtils.SIAPENDIENTE_SERVICIO_BORRADO) == 0) {
			...
			// Envia a SIA el borrado
			final SiaResultado siaResultado = borradoServicio(siaPendiente.getIdElemento(),
					siaPendiente.getIdSia().toString(), siaPendiente.getSiaUA());
			...
			final Servicio servicio = servicioDelegate.obtenerServicioParaSolr(siaPendiente.getIdElemento(), null);
			if (servicio == null) {
	...
	private SiaResultado borradoProcedimiento(final Long idProc, final String idSIA, final SiaUA siaUA)
			throws Exception {
		SiaResultado resultado = null;
		...
		try {
			...
			log.error("Se ha producido un error enviando el dato a SIA de un borrado de procedimiento " + idProc,
					exception);
			throw new Exception(
	...
	private SiaResultado borradoServicio(final Long idServ, final String idSIA, final SiaUA siaUA) throws Exception {
		SiaResultado resultado = null;
		final Sia sia = new Sia();
		sia.setIdSIA(idSIA);
		sia.setOperacion(SiaUtils.ESTADO_BAJA);
		...
		try {
			...
			throw new Exception("Se ha producido un error enviando el dato a SIA de un borrado de servicio " + idServ,
					exception);
		}
		return resultado;
	}
	 ...
	 * @return
	 */
	private int calcularDias(final SiaPendiente siaPendiente) {
		final int dias = (int) ((new Date().getTime() - siaPendiente.getFecAlta().getTime()) / 86400000);
		return dias;
	}
	 ...
	 * @throws DelegateException
	 */
	public void info(final SiaJob siaJob) {
		log.debug("Obtiene la info de todos los procedimientos y extrae la información SIA.");

		final ProcedimientoDelegate procDelegate = DelegateUtil.getProcedimientoDelegate();

		final StringBuffer resultadoDescripcion = new StringBuffer();
		final StringBuffer resultadoDescripcionBreve = new StringBuffer();
		final Calendar calendar = Calendar.getInstance();

		final int correctos = 0;
		int incorrectos = 0;
		try {
			final List<Long> idProcedimientos = procDelegate.buscarIdsProcedimientos();
			for (final Long idProcedimiento : idProcedimientos) {
				...
				try {
					if ((correctos + incorrectos) % 50 == 0) {
						final StringBuffer resultadoBeta = new StringBuffer();
						resultadoBeta.append(
								"Se han revisado info de " + (correctos + incorrectos) + " datos de un total de "
										+ idProcedimientos.size() + " procedimientos. De los cuales: <br />");
						resultadoBeta.append(" - " + correctos + " datos correctos  <br />");
						resultadoBeta.append(" - " + incorrectos + " datos incorrectos  <br />");
						resultadoBeta.append("Estado: "
								+ Math.abs((correctos + incorrectos) * 100 / idProcedimientos.size()) + "%  <br />");
						...
						final SiaPendienteProcesoDelegate siaPendienteDelegate = DelegateUtil
								.getSiaPendienteProcesoDelegate();
						siaPendienteDelegate.actualizarSiaJob(siaJob);
				...
				/*
				 * if (SiaUtils.isEnviableSia(procedimiento)) { try {
				 * enviarProcedimiento(procDelegate, procedimiento, null, resultadoDescripcion,
				 * correctos, incorrectos); } catch (Exception e) {
				 * log.error("Error general enviando procedimiento ",e); incorrectos++;
				 * resultadoDescripcion.
				 * append("   -- Se ha enviado a SIA incorrectamente. <br />");
				 * resultadoDescripcion.append("   -- Mensaje:"+ExceptionUtils.getStackTrace(e)
				 * +" <br />"); }
				 *
				 * }
				 */
		...
		final SimpleDateFormat formato = new SimpleDateFormat("H:mm");
		resultadoDescripcionBreve
				.append("Se han revisado info de " + (correctos + incorrectos) + " datos. De los cuales: <br />");
	 ...
	 * @throws Exception
	 */
	private Sia obtenerSiaProcedimiento(final ProcedimientoLocal procedimiento) throws Exception {
		final SiaEnviableResultado siaEnviableResultado = SiaUtils.isEnviable(procedimiento);
	 ...
	 * @throws Exception
	 */
	private Sia obtenerSiaProcedimientoNoActivo(final ProcedimientoLocal procedimiento) throws Exception {
	...
	/**
	 * Nucleo encargado de obtener el objeto SIA a partir de los datos que se les
	 * pasa.
	 *
	 * @param procedimiento
	 ...
	 * @throws Exception
	 */
	private Sia obtenerSiaProcedimiento(final ProcedimientoLocal procedimiento,
			final SiaEnviableResultado siaEnviableResultado, final SiaCumpleDatos siaCumpleDatos) throws Exception {
		final Sia sia = new Sia();
		...
		} else if (procedimiento.getUnidadAdministrativa().getTraduccion("es") != null
				&& ((TraduccionUA) procedimiento.getUnidadAdministrativa().getTraduccion("es")).getNombre() != null) {
		...
		if (esProcSerInterno) {
			...
			final String[] destinatarios = new String[procedimiento.getPublicosObjetivo().size()];
			final Set<PublicoObjetivo> publicoObjs = procedimiento.getPublicosObjetivo();
			int i = 0;
			for (final PublicoObjetivo pObj : publicoObjs) {
		...
		for (final Tramite tramite : tramites) {
			if (tramite != null && tramite.getFase() == 1) {
				...
				// Revisar no inicializa la lista dentro de tramite aunq en el servicio que
				// obtiene proc si lo hace...
				final TramiteDelegate tramDelegate = DelegateUtil.getTramiteDelegate();
				final Tramite tram = tramDelegate.obtenerTramite(tramite.getId());

				final Set<DocumentTramit> docs = tram.getDocsInformatius();
				docs.addAll(tram.getDocsRequerits());

				for (final DocumentTramit documentTramit : docs) {
		...
		if (SiaUtils.isActivoSiaComunes()) {
			esComunProc = procedimiento.isComun();
		}
	 ...
	 * @throws Exception
	 */
	private Sia obtenerSiaServicioNoActivo(final Servicio servicio) throws Exception {
	 ...
	 * @throws Exception
	 */
	private Sia obtenerSiaServicio(final Servicio servicio) throws Exception {
		final SiaEnviableResultado siaEnviableResultado = SiaUtils.isEnviable(servicio);
		final SiaCumpleDatos siaCumpleDatos = SiaUtils.cumpleDatos(servicio, siaEnviableResultado, true);
		return obtenerSiaServicio(servicio, siaEnviableResultado, siaCumpleDatos);
	}
	...
	/**
	 * Nucleo del obtener SIA a partir de servicio, SiaEnviableResultado y
	 * SiaCumpleDatos.
	 *
	 * @param servicio
	 * @return
	 * @throws Exception
	 */
	private Sia obtenerSiaServicio(final Servicio servicio, final SiaEnviableResultado siaEnviableResultado,
			final SiaCumpleDatos siaCumpleDatos) {
		final Sia sia = new Sia();
		sia.setIdElemento(servicio.getId().toString());
		if (servicio.getCodigoSIA() != null) {
			sia.setIdSIA(servicio.getCodigoSIA());
		}
		...
		if (servicio.isComun()) {
			sia.setUaGest(RolsacPropertiesUtil.getUAComun(false));
		} else if (servicio.getOrganoInstructor().getTraduccion("es") != null
				&& ((TraduccionUA) servicio.getOrganoInstructor().getTraduccion("es")).getNombre() != null) {
		...
		if (esProcSerInterno) {
			...
			final String[] destinatarios = new String[servicio.getPublicosObjetivo().size()];
			final Set<PublicoObjetivo> publicoObjs = servicio.getPublicosObjetivo();
			int i = 0;
			for (final PublicoObjetivo pObj : publicoObjs) {
				switch (pObj.getId().intValue()) {
				case 200:
		...
		if (servicio.getMaterias() != null) {
			for (final Materia mat : servicio.getMaterias()) {
				if (mat.getCodigoSIA() != null) {
					if (!materias.contains(mat.getCodigoSIA().toString())) {
						materias.add(mat.getCodigoSIA().toString());
					}
		...
		// Se ha tenido que poner aquí (y se ha simplificado) pq se produce un error al
		...
		if (servicio.getTramiteUrl() != null) {
			sia.setNivAdm("4");
		} else {
			sia.setNivAdm("1");
		}
