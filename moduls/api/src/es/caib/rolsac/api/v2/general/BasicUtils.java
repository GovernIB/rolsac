...
public class BasicUtils {
	...
	public static List<CriteriaObject> parseCriterias(final Class<?> criteriaClass, final String entityAlias,
			final BasicCriteria basicCriteria) throws CriteriaObjectParseException {
		return parseCriterias(criteriaClass, entityAlias, "", basicCriteria);
	}

	/**
	 * Use case: if (procedimentCriteria.getTaxa() != null) {
	 * ProcedimentByTaxaCriteria criteria = new
	 ...
	 */
	public static List<CriteriaObject> parseCriterias(final Class<?> criteriaClass, final String entityAlias,
			final String i18nAlias, final BasicCriteria basicCriteria) throws CriteriaObjectParseException {
		final List<CriteriaObject> criteriaObjects = new ArrayList<CriteriaObject>();

		parseBasicCriteria(entityAlias, i18nAlias, criteriaObjects, basicCriteria);

		final Method[] methods = criteriaClass.getDeclaredMethods();
		Method m = null;
		for (int i = 0; i < methods.length; i++) {
			try {
				...
				if ((m.getName().startsWith("get") || m.getName().startsWith("is"))
					...
					final Object value = m.invoke(basicCriteria);
					if (value != null) {
						final CriteriaObject co = getParsedCriteria(m, value, criteriaClass, entityAlias, i18nAlias,
								basicCriteria);
						if (co != null) {
				...
				throw new CriteriaObjectParseException(
						"Error parseando " + m.getName() + " de la clase " + criteriaClass + ".", e);
			}
		}
	...
	private static CriteriaObject getParsedCriteria(final Method getter, final Object value,
			final Class<?> criteriaClass, final String entityAlias, final String i18nAlias,
			final BasicCriteria basicCriteria)
		...
		/*
		 * Obtener la clase CriteriaObject especifica. Nota: Los criterias objects se
		 * encuentran en packages del tipo
		 * "es.caib.rolsac.api.v2.fetVital.co.FetVitalByCodigoEstandarCriteria".
		 */
		final StringBuilder concreteCriteriaClassName = new StringBuilder();
		final String[] packageWords = criteriaClass.getName().split("\\.");

		// split by unicode capital letter.
		final String[] classWords = packageWords[packageWords.length - 1].split("(?=\\p{Lu})");
		...
		final Class<?> concreteCriteriaClass = Class.forName(concreteCriteriaClassName.toString());
		final Constructor<?> ct = concreteCriteriaClass.getConstructors()[0];
		...
		if (concreteCriteria != null) {
			...
			final Method parser = concreteCriteriaClass.getMethod("parseCriteria", String.class);
	...
	private static void parseBasicCriteria(final String entityAlias, final String i18nAlias,
			final List<CriteriaObject> criteriaObjects, final BasicCriteria basicCriteria)
			throws CriteriaObjectParseException {

		if (StringUtils.isNotBlank(basicCriteria.getId())) {
			final BasicByIdCriteria criteria = new BasicByIdCriteria(entityAlias);
			criteria.parseCriteria(basicCriteria.getId());
			criteriaObjects.add(criteria);
		}
		final String ordenar = controlOrdenar(basicCriteria);
		if (StringUtils.isNotBlank(ordenar)) {
			final BasicByOrdenacioCriteria criteria = new BasicByOrdenacioCriteria(entityAlias, i18nAlias);
			criteria.parseCriteria(ordenar);
			criteriaObjects.add(criteria);
		}
		if (StringUtils.isNotBlank(basicCriteria.getInici())) {
			final BasicByIniciCriteria criteria = new BasicByIniciCriteria();
			criteria.parseCriteria(basicCriteria.getInici());
			criteriaObjects.add(criteria);
		}
		if (StringUtils.isNotBlank(basicCriteria.getTamany())) {
			final BasicByTamanyCriteria criteria = new BasicByTamanyCriteria();
	...
	public static Object entityToDTO(final Class<?> dtoClass, final Object entity) {
		return entityToDTO(dtoClass, entity, null);
	}

	public static Object entityToDTO(final Class<?> dtoClass, final Object entity, final String lang) {
		if (entity == null) {
			return null;
		}
		...
		try {
			// Para asegurarnos de que es devuelto el constructor por defecto, ya que AXIS
			// genera objetos
		...
		try {
			...
			final Method[] dtoMethods = dtoClass.getMethods();
			Method method;

			for (i = 0; i < dtoMethods.length; i++) {
				method = dtoMethods[i];
				if (method.getName().startsWith("set") && !Arrays.asList(SETTERS_INVALIDS).contains(method.getName())) {
	...
	private static void copyProperty(final Object entity, final Object dto, final Method setter, final String lang)
			throws SecurityException, NoSuchMethodException, IllegalArgumentException, IllegalAccessException,
			InvocationTargetException, NoSuchFieldException {

		final String property = StringUtils.uncapitalize(setter.getName().substring(3));
		Object value = null;
		...
		try {
			...
			if (value == null) {
				...
				final String[] langAlternates = getLangAlternates(lang);
		...
		if (value == null) {
			...
			try {
			...
			} catch (final NoSuchMethodException eGet) {
				...
				try {
					...
					value = entityGetter.invoke(entity);
				} catch (final NoSuchMethodException eIs) {
				}
			}
		}
		...
		if (value != null) {
			try {
				final Method idGetter = value.getClass().getMethod("getId");
		...
		if (value != null) {
			...
			final Class<?> propertyClass = dto.getClass().getDeclaredField(property).getType();
			final Class<?>[] valueClasses = new Class[1];
			if (propertyClass.equals(Boolean.class) || propertyClass.equals(boolean.class)) {
				...
				final Long tmpValue = ((Timestamp) value).getTime();
			...
			final Method dtoSetter = MethodUtils.getMatchingAccessibleMethod(dto.getClass(),
	...
	private static String[] getLangAlternates(String lang) {
		...
		final String alternates = System.getProperty("es.caib.rolsac.api.v2.alternativesIdioma_" + lang);
		if (alternates == null) {
	...
	public static String controlOrdenar(final BasicCriteria criteria) {

		String ordenaciones = new String();
		try {
			final Method idGetter = criteria.getClass().getMethod("getOrdenar");
			final Object[] objetos = (Object[]) idGetter.invoke(criteria);
			...
			for (final Object objeto : objetos) {
				final String[] aux = objeto.toString().split("_");
				if (aux.length == 2) {
			...
			final Field field = criteria.getClass().getDeclaredField("ordenar");
