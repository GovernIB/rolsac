...
<html>
<head>
    <title><spring:message code='arbreET.arbre'/></title>
    ...
    <link href="<c:url value='/css/arbreET.css'/>" rel="stylesheet" type="text/css" media="screen" />
    ...
    <!--
        ...
        function carregar(id, nombre){
            ...
            <c:choose>
                ...
                <c:otherwise>
                    ...
                    window.top.ModulEspais.agregaItem(etItem);
                    window.top.ModulEspais.inicializarEspacios();
                </c:otherwise>
            </c:choose>
        ...
        <c:if test='${not empty raizOptions}'>
	        ex.push(new init_ex(1, "", "<spring:message code='espai.arrel' />", false, ""), true);
                ...
                <c:if test="${idET != idActual}">
                    ...
                    <c:if test="${not empty coll}" >
                        <c:import url = "/pantalles/popFillsET.jsp">
                            ...
                            <c:param name = "inicial" value = "${idET}" />
                        </c:import>
                    </c:if>
                </c:if>         
            </c:forEach>
        ...
        function unopen(nr){
            open(nr);
            //window.location = "<c:url value='popArbreETContreure.do' />?idET=<c:out value='${idET}' />&idSelect=" + ex[nr].codi + "&idHidden=<c:out value='${id_hidden}' />&idInput=<c:out value='${id_input}' />";
        }

        

        function open(nr){
            window.location = "<c:url value='popArbreETExpandir.do' />?idET=<c:out value='${idET}' />&idSelect=" + ex[nr].codi + "&idHidden=<c:out value='${id_hidden}' />&idInput=<c:out value='${id_input}' />";
        }
     -->   
    </script>
</head>
...
<body>
    <div id="escript">
        ...
        <!-- <div id="modulPrincipal" class="modulPrincipal"> -->
            ...
            <div class="modul" >
                <fieldset>
                    <legend><spring:message code='arbreET.llistat'/></legend>
                    <div class="modul_continguts mostrat">
                        <c:if test='${not empty raizOptions}'>
                            ...
                            <form action='<c:url value="/pantalles/popArbreET.do"/>' id="padreForm" method="post" >
                        ...
                        <c:if test='${empty raizOptions}'>
                            <center>
                                <spring:message code='arbreET.buit'/>
                            </center>
                        </c:if>
                        <div class="botonera">
                            <div class="btnGenerico">
                               <a href="javascript:borrarPopUp('popET')" class="btn tancar"><span><span><spring:message code='boto.tancar'/></span></span></a>
                           </div>
                        </div>                              
                    </div>                              
                </fieldset>                         
