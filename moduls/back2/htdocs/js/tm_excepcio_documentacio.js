...
//						codi_cap1 = "<div class=\"th nom" + ordre_c1 + "\" role=\"columnheader\"><a href=\"javascript:;\">" + txtLlistaItem + "</a></div>";
			...
			$(data.nodes).each(function(i) {
				...
				codi_taula += "<div class=\"td excepcioDocumentacio\" role=\"gridcell\">";
				codi_taula += "<input type=\"hidden\" value=\"" + dada_node.id + "\" class=\"id\" />";
				codi_taula += "<a id=\"excepcioDocumentacio_"+dada_node.id+"\" href=\"javascript:;\" class=\"nom\">" + printStringFromNull(dada_node.nom, txtSinValor) + "</a>";
				codi_taula += "</div></div>";
			});
...
function CDetall(){
	...
	this.pintar = function(dades) {
		...
		for (var i in idiomas) {
			...
			$("#item_descri_" + idioma).val(printStringFromNull(dada_node[idioma]["descripcion"]));
		}
