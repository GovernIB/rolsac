...
// 			codi_cap1 = "<div class=\"th nom" + ordre_c1 + "\" role=\"columnheader\"><a href=\"javascript:;\">" + txtLlistaItem + "</a></div>";
			...
			codi_cap1 = "<div class=\"th nom" + ordre_c1 + "\" role=\"columnheader\">" + txtLlistaItems + "</div>";
			...
			$(data.nodes).each(function(i) {
				...
				codi_taula += "<a id=\"nom_"+dada_node.id+"\" href=\"javascript:;\" class=\"nom\">" + printStringFromNull(dada_node.nombre, txtSinValor) + "</a>";
				codi_taula += "</div>";				
				codi_taula += "<div class=\"td descripcio\" role=\"gridcell\">" + printStringFromNull(dada_node.descripcion, txtSinValor) + "</div>";
	...
	this.carregar = function(opcions) {
		...
		if (typeof opcions.cercador != "undefined" && opcions.cercador == "si") {
			...
			dataVars_cercador += "&admresp=" + $("#cerca_admresp").val();
			dataVars_cercador += "&excepcio=" + $("#cerca_excepcio").val();
		} else {
...
function CDetall(){
	...
	this.iniciar = function() {	
        ...
        jQuery("#item_admresp,#item_admresp_es,#item_admresp_ca,#item_admresp_en,#item_admresp_de,#item_admresp_fr").change(function(){
            jQuery("#item_admresp,#item_admresp_es,#item_admresp_ca,#item_admresp_en,#item_admresp_de,#item_admresp_fr").val( jQuery(this).val() );
        });
	...
	this.pintar = function(dades) {
		...
		jQuery("#item_admresp").val(dada_node.item_admresp);
		jQuery("#item_admresp").change();
		jQuery("#item_excepcio").val(dada_node.item_excepcio);
		jQuery("#item_excepcio").change();
