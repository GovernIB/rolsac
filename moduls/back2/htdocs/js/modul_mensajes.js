...
function abrirMensaje(idDato) {
	...
	function recuperarMensaje(data,i){
		...
		if (tienePermisoPublicar != 'S') {
		  ...
		  var  creado;
		  if (data.mensajes[i].usuarioNombre == null) {
			  creado = " " + data.mensajes[i].usuario;
		  } else {
			  creado = data.mensajes[i].usuarioNombre + " " + data.mensajes[i].usuario;
		  }
		  var  leidomsg = data.mensajes[i].usuarioLecturaNombre + " " + data.mensajes[i].usuarioLectura;
