 ...
 */
function DetallBase_n(soloFicha, reglasFormulario, identificadores) {
	...
	//DIV escriptori_contenido
	this.escriptori_contingut_elm = null;
	//DIV escriptori_elemento
	this.escriptori_detall_elm = null;
	...
	this.iniciarBotones = function () {
		jQuery("#btnEliminar").unbind("click").bind("click",function(){ 
			Detall.eliminar(); 
		});
		jQuery("#btnPrevisualizar").unbind("click").bind("click",function(){ 
			Detall.previsualitza();
		});
		jQuery("a#btnGuardar").unbind("click").bind("click", function() {
			Detall.guarda();
		});
		jQuery("a#btnNuevaFicha").unbind("click").bind("click", function(){ 
			lista.nuevaFicha(); 
		});
		
	};
	this.LANG_TRADUCTOR = "ca";
	this.Error = new CError();
	this.iniciarTinyMce = function () {
	...
	this.guardaGenerico = function(dataVars, funcion) {
		...
		$.ajax({
			...
			success: function(data) {
				...
				//this.Llistat.cacheDatosListado = null;
	...
	this.modificado = function(marcar) {
		...
		jQuery("#"+ids.btnGuardar).unbind("click").bind("click", function() { 
			that.guarda(); 
		} ).parent().toggleClass("off", !marcar);
	...
	this.cierra = function() {
		...
		this.escriptori_detall_elm.fadeOut(300, function() {
			that.escriptori_contingut_elm.fadeIn(300);
		});
	...
	this.Llistat = null;
	this.setLlistat = function (listado) {
		this.Llistat = listado;
	};
	...
	this.carregar = function(itemID) {
		...
		this.escriptori_detall_elm.find(".botonera li.btnEliminar,.botonera li.btnPrevisualizar").show();

		if (itemID == undefined) {

			itemID = $("#item_id").val();
			this.Llistat.itemID = itemID;

		}

		this.escriptori_contingut_elm.fadeOut(300, function() {

			//codi_carregant = "<div id=\"carregantDetall\"><p class=\"executant\">" + txtCarregantDetall + "</p></div>";
			//that.escriptori_detall_elm.append(codi_carregant).slideDown(300, function() {
			that.escriptori_detall_elm.slideDown(300, function() {
	...
	this.idioma = function(e) {
		...
		if (elm.is("A")) {
			...
			if (!elm.hasClass("desplegar")) {
				...
				if (this.escriptori_detall_elm.find("div.idiomes").size() > 1) {
				...
				this.escriptori_detall_elm.find(".element.multilang .campoIdioma").hide();
				this.escriptori_detall_elm.find(".element.multilang ." + a_clicat_class).show();
	...
	this.previsualitza = function() {
		...
		var idiomaSeleccionat = this.escriptori_detall_elm.find("ul.idiomes li.seleccionat span").attr("class");
		var id = this.escriptori_detall_elm.find("#item_id").val();
