	...
	this.finCargaListado = function(opcions,data) {
		...
		if (resultats_total > 0) {
			...
			$(data.nodes).each(function(i) {			
				...
				codi_taula += "<input type=\"hidden\" value=\"" + dada_node.lang + "\" class=\"lang\" />";
				codi_taula += "<span class=\"ordre\">" + (printStringFromNull(dada_node.orden, txtSinValor)) + "</span>";
				codi_taula += "<a id=\"idioma_"+dada_node.lang+"\" href=\"javascript:;\" class=\"idioma editarIdioma\">" + printStringFromNull(dada_node.nombre, txtSinValor) + "</a>";
				codi_taula += "</div>";
				codi_taula += "<div class=\"td enllas\" role=\"gridcell\">";

				codi_taula += that.getHtmlSelectorOrdenacion("idioma_"+dada_node.lang, dada_node.orden-1, resultats_total );
...
*/
	this.pintar = function(dades) {
		...
		dada_node = dades;
		jQuery("#item_lang").val(dada_node.item_lang);
		jQuery("#item_codi_estandard").val(dada_node.item_codi_estandard);
        jQuery("#item_codi_estandard").change();
        jQuery("#item_nom").val(dada_node.item_nom);
        jQuery("#item_lang_traductor").val(dada_node.item_lang_traductor);
