...
public class TMFetsVitalsController extends PantallaBaseController {
	...
	public @ResponseBody Map<String, Object> llistat(final HttpServletRequest request) {
		...
		final List<Map<String, Object>> llistaFetsVitalsDTO = new ArrayList<Map<String, Object>>();
		...
		try {
			...
			final HechoVitalDelegate fetsVitalsDelegate = DelegateUtil.getHechoVitalDelegate();
			final String idiomaPorDefecto = DelegateUtil.getIdiomaDelegate().lenguajePorDefecto();

			resultadoBusqueda = fetsVitalsDelegate.listarHechosVitales(Integer.parseInt(pagPag),
					Integer.parseInt(pagRes), idiomaPorDefecto);

			for (final Object o : resultadoBusqueda.getListaResultados()) {

				final Long id = (Long) ((Object[]) o)[0];
				final int ordre = ((Integer) ((Object[]) o)[1]).intValue();
				final String nom = (String) ((Object[]) o)[2];
	...
	public @ResponseBody Map<String, Object> recuperaDetall(final Long id, final HttpServletRequest request) {
		...
		try {
			...
			final HechoVitalDelegate fetsVitalsDelegate = DelegateUtil.getHechoVitalDelegate();
			final HechoVital fetsVitals = fetsVitalsDelegate.obtenerHechoVital(id);
			...
			if (fetsVitals.getIconoGrande() != null) {
				resultats.put("item_icona_gran_enllas_arxiu",
						"fetsVitals/archivo.do?id=" + fetsVitals.getId() + "&tipus=3");
	...
	public @ResponseBody Map<String, Object> recuperaModulos(final Long id, final HttpServletRequest request) {
		...
		try {
			...
			final HechoVitalDelegate fetsVitalsDelegate = DelegateUtil.getHechoVitalDelegate();
			final HechoVital fetVital = fetsVitalsDelegate.obtenerHechoVital(id);

			final List<HechoVitalProcedimiento> lista = castList(HechoVitalProcedimiento.class,
					fetVital.getHechosVitalesProcedimientos());
			final List<HechoVitalProcedimiento> listaProcedimientos = new LinkedList<HechoVitalProcedimiento>();

			// Procedimientos que pasaremos como resultado.
			final List<ProcedimientoLocalDTO> procedimientos = new LinkedList<ProcedimientoLocalDTO>();

			// Evitamos posibles nulos iniciales devueltos en la variable "lista" por
			// "fetVital.getHechosVitalesProcedimientos()".
			for (final HechoVitalProcedimiento hechoProc : lista) {
				if (hechoProc != null)
			...
			for (final HechoVitalProcedimiento hvProc : listaProcedimientos) {

				final TraduccionProcedimientoLocal traProc = (TraduccionProcedimientoLocal) hvProc.getProcedimiento()
						.getTraduccion(lang);

				procedimientos.add(new ProcedimientoLocalDTO(hvProc.getId(), traProc != null ? traProc.getNombre() : "",
						null, null, null, hvProc.getOrden(), hvProc.getProcedimiento().getId(), // Related item id
						id // Main item id
				));
	...
	private void omplirCampsTraduibles(final Map<String, Object> resultats, final HechoVital fetsVitals)
			throws DelegateException {

		final IdiomaDelegate idiomaDelegate = DelegateUtil.getIdiomaDelegate();
		final List<String> langs = castList(String.class, idiomaDelegate.listarLenguajes());

		for (final String lang : langs) {

			if (null != fetsVitals.getTraduccion(lang)) {

				final HashMap<String, String> traduccionFetsVitalsDTO = new HashMap<String, String>();
				final TraduccionHechoVital tm = (TraduccionHechoVital) fetsVitals.getTraduccion(lang);
				...
				if (tm.getDistribComp() != null) {
					traduccionFetsVitalsDTO.put("distribucio_enllas_arxiu",
							"fetsVitals/archivo.do?id=" + fetsVitals.getId() + "&lang=" + lang + "&tipus=4");
				...
				if (tm.getNormativa() != null) {
					traduccionFetsVitalsDTO.put("normativa_enllas_arxiu",
							"fetsVitals/archivo.do?id=" + fetsVitals.getId() + "&lang=" + lang + "&tipus=5");
				...
				if (tm.getContenido() != null) {
					traduccionFetsVitalsDTO.put("contingut_enllas_arxiu",
							"fetsVitals/archivo.do?id=" + fetsVitals.getId() + "&lang=" + lang + "&tipus=6");
	...
	public ResponseEntity<String> guardar(final HttpSession session, final HttpServletRequest request) {
		...
		final HechoVital fetVital = new HechoVital();
		...
		try {
			...
			// Iremos recopilando los parámetros de tipo fichero en el Map ficherosForm y el
			...
			final boolean edicion = valoresForm.get("item_id") != null && !"".equals(valoresForm.get("item_id"));
			if (edicion) {
			...
			for (final String idioma : idiomas) {
				...
				} else if (valoresForm.get("item_distribucio_" + idioma + "_delete") != null
						&& !"".equals(valoresForm.get("item_distribucio_" + idioma + "_delete"))) {
				...
				} else if (valoresForm.get("item_normativa_" + idioma + "_delete") != null
						&& !"".equals(valoresForm.get("item_normativa_" + idioma + "_delete"))) {
				...
				} else if (valoresForm.get("item_contingut_" + idioma + "_delete") != null
						&& !"".equals(valoresForm.get("item_contingut_" + idioma + "_delete"))) {
			...
			} else if (valoresForm.get("item_icona_delete") != null
					&& !"".equals(valoresForm.get("item_icona_delete"))) {
			...
			} else if (valoresForm.get("item_icona_gran_delete") != null
					&& !"".equals(valoresForm.get("item_icona_gran_delete"))) {
				fetVital.setIconoGrande(null);
			}

			fetsVitalsDelegate.grabarHechoVital(fetVital);

			result = new IdNomDTO(fetVital.getId(),
					messageSource.getMessage("fetVital.guardat.correcte", null, request.getLocale()));
			...
			final String error = messageSource.getMessage("error.fitxer.tamany", null, request.getLocale());
	...
	public @ResponseBody IdNomDTO pujarFetsVitals(final HttpServletRequest request) {
		final IdNomDTO resultatStatus = new IdNomDTO();
		try {
			final Long id = new Long(request.getParameter("id"));
			final HechoVitalDelegate fetsVitalsDelegate = DelegateUtil.getHechoVitalDelegate();
	...
	public @ResponseBody IdNomDTO reordenarFetsVitals(final HttpServletRequest request) {
		final IdNomDTO resultatStatus = new IdNomDTO();

		try {

			final Long id = new Long(request.getParameter("id"));
			final Integer ordenNuevo = new Integer(request.getParameter("orden"));
			final Integer ordenAnterior = new Integer(request.getParameter("ordenAnterior"));

			final HechoVitalDelegate hechoVitalDelegate = DelegateUtil.getHechoVitalDelegate();
	...
	public @ResponseBody Map<String, Object> traduir(final HttpServletRequest request) {
		...
		try {
			...
			final TraduccionHechoVital traduccioOrigen = getTraduccionOrigen(request, idiomaOrigenTraductor);
	...
	private TraduccionHechoVital getTraduccionOrigen(final HttpServletRequest request,
			final String idiomaOrigenTraductor) {

		final TraduccionHechoVital traduccioOrigen = new TraduccionHechoVital();
	...
	public @ResponseBody IdNomDTO guardarProcedimientos(final Long id, final Long[] elementos,
		 ...
		 * petición es invocada desde un iFrame (oculto) algunos navegadores interpretan
		...
		final List<Long> hvProcIds = new LinkedList<Long>();
		final List<HechoVitalProcedimiento> hvpAGrabar = new LinkedList<HechoVitalProcedimiento>();
		...
		try {
			...
			final List<HechoVitalProcedimiento> listaHechoVitalProcedimientos = fetVitalOld
					.getHechosVitalesProcedimientos();
			for (final HechoVitalProcedimiento hvProc : listaHechoVitalProcedimientos) {
				if (hvProc != null)
					hvProcIds.add(hvProc.getId());
			}

			// Es posible que hayan vaciado de elementos el módulo. En ese caso, elementos
			// será null.
			if (elementos != null) {
				...
				for (int i = 0; i < elementos.length; i++) {
					...
					final long procedimientoId = elementos[i];
			...
			result = new IdNomDTO(fetVitalOld.getId(),
					messageSource.getMessage("fetVital.guardat.procediments.correcte", null, request.getLocale()));
