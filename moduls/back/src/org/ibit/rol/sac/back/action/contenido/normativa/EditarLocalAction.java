...
/**
 * Action para editar una Normativa Local.
 *
 * @struts.action
 *  name="normativaForm"
 *  scope="request"
 *  validate="true"
 *  path="/contenido/normativa/local/editar"
 *  input=".contenido.normativa.local"
 *  parameter="action"
 *
 * @struts.action
 *  name="normativaForm"
 *  scope="request"
 *  validate="false"
 *  path="/contenido/normativa/local/seleccionar"
 *  parameter="action"
 *
 * @struts.action-forward
 *  name="success" path=".contenido.normativa.local"
 *
 * @struts.action-forward
 *  name="cancel" path="/contenido/normativa/local.do"
 *
 * @struts.action-forward
 ...
 *
 */
public class EditarLocalAction  extends BaseDispatchAction {

    protected static Log log = LogFactory.getLog(EditarLocalAction.class);
    private static String TIPO_NORM = "local";
    ...
    /**
     * Método que traduce un formulario de Normativa Local
     * @author	Indra
    ...
    public ActionForward seleccionar(ActionMapping mapping, ActionForm form, HttpServletRequest request,
        ...
        if (id != null){
            ...
            if(normativa instanceof NormativaExterna){
                return mapping.findForward("fail");
            }
            NormativaLocal normativaLocal = (NormativaLocal)normativa;
            VOUtils.describe(dForm, normativaLocal);

            dForm.setFecha(normativaLocal.getFecha());
            dForm.setFechaBoletin(normativaLocal.getFechaBoletin());

            if (normativaLocal.getNumero() != null)
            dForm.set("valorNumero", new Long(normativaLocal.getNumero()));
            
            if (normativaLocal.getRegistro() != null)
            	dForm.set("valorRegistro", new Long(normativaLocal.getRegistro()));

            if (normativaLocal.getBoletin() != null){
                dForm.set("idBoletin", normativaLocal.getBoletin().getId());
                dForm.set("nombreBoletin", normativaLocal.getBoletin().getNombre());
            }

            if (normativaLocal.getTipo() != null){
                dForm.set("idTipo", normativaLocal.getTipo().getId());
                dForm.set("nombreTipo", ((TraduccionTipo)normativaLocal.getTipo().getTraduccion()).getNombre());
            }

            if (normativaLocal.getUnidadAdministrativa() != null){
                dForm.set("idUA", normativaLocal.getUnidadAdministrativa().getId());
                dForm.set("nombreUA", ((TraduccionUA)normativaLocal.getUnidadAdministrativa().getTraduccion()).getNombre());
            }

            request.setAttribute("afectacionOptions", normativaLocal.getAfectadas());
            request.setAttribute("procedimientoOptions", normativaLocal.getProcedimientos());
        }
    ...
    public ActionForward busqueda(ActionMapping mapping, ActionForm form,
        ...
        if (dForm.get("idTipo") != null){
        ...
        } else if (dForm.get("idUA") != null){
            Long idUA = (Long) dForm.get("idUA");
            normativas = normativaDelegate.buscarNormativasUA(idUA, TIPO_NORM);
            ...
            while (itLang.hasNext()){
                TraduccionNormativa tNormativa = (TraduccionNormativa) itTrad.next();
                int x = 0;
