...
import org.ibit.rol.sac.back.action.contenido.ficha.RelacionarMateriaAction;
...
import org.ibit.rol.sac.back.negocio.GrabadorFicha;
...
public class EditarFichaAction extends BaseDispatchAction{
	...
	protected Map<String,String> getKeyMethodMap(){
		...
		map.put("ficha.relacion.materias", "relacionarMateria");
	...
	private boolean esUnaFichaNueva(Long id) {
	...
	public ActionForward editar(ActionMapping mapping, ActionForm form,
		...
		UnidadAdministrativaDelegate uaDelegate = getUADelegate();
		Ficha ficha = null;
		...
		try {
			...
			if (esUnaFichaNueva(id)) {
				...
				ficha = fichaDelegate.obtenerFicha(id);
			...
			VOUtils.populate(ficha, dForm, getIdiomaDelegate());
			...
			if(estaCampoRellenado(urlMicrosite) && !esUrlAbsoluta(urlMicrosite)) {
				urlMicrosite = crearUrlAbsoluta(urlMicrosite);
				establirCampoUrlMicrosite(dForm, urlMicrosite);
			}
			...
			if (request.getParameter("action").equals(getResources(request).getMessage(request.getLocale(), "boton.traducir"))) {
				...
				getGrabadorFicha().guardar(dForm,ficha);
			...
			if (id == null) request.setAttribute("alert", "ficha.aviso.privilegio.insertar");
	...
	private boolean estaCampoRellenado(String campo) {
		if(null==campo) return false;
	...
	private String obtenerCampoUrlMicrosite(FichaForm dform) {
		TraduccionFicha traduccion = (TraduccionFicha)dform.getCatala();
		return traduccion.getUrl();
	}


	private String crearUrlAbsoluta(String urlRelativa) {
		return System.getProperty("es.caib.rolsac.portal.url") + urlRelativa;
	}
	...
	private void establirCampoUrlMicrosite(FichaForm dform, String urlMicrosite) {
		TraduccionFicha traduccion = (TraduccionFicha)dform.getCatala();
		traduccion.setUrl(urlMicrosite);
	...
	public ActionForward seleccionar(ActionMapping mapping, ActionForm form, HttpServletRequest request,
		...
		if (id != null){
			...
			VOUtils.describe(dForm, ficha,getIdiomaDelegate());
			...
			log.debug("fichaId "+ficha.getId()+ " esta clasificada? " + ficha.estaClasificado());

			List<Materia> materias=new ArrayList<Materia>(ficha.getMaterias());
			List<Materia> materiasFiltradas = getBuscadorMaterias().listarMateriasFiltrandoSinClasificar(materias);
	...
	public ActionForward relacionarMateria(ActionMapping mapping, ActionForm form,
		...
		return getFichaRelacionarMateriaAction().relacionarMateria(this, new ActionParameters(mapping, form, request, response));
	...
	public IdiomaDelegate getIdiomaDelegate() {
		if(null==idiomaDelegate) 
			idiomaDelegate = DelegateUtil.getIdiomaDelegate();
	...
	//
	private UnidadAdministrativaDelegate uaDelegate;
	...
	//
	private GrabadorFicha grabadorFicha;


	public GrabadorFicha getGrabadorFicha() {
		if(null==grabadorFicha) grabadorFicha=new GrabadorFicha();
		return grabadorFicha;
	}

	public void setGrabadorFicha(GrabadorFicha grabadorFicha) {
		this.grabadorFicha = grabadorFicha;
	}

	//
	private RelacionarMateriaAction relacionarMateriaAction; 

	public void setFichaRelacionarMateriaAction(RelacionarMateriaAction relacionarMateriaAction) {
		this.relacionarMateriaAction = relacionarMateriaAction;
	}

	public RelacionarMateriaAction getFichaRelacionarMateriaAction() {
	...
	//
	private BuscadorMaterias buscadorMaterias;
	...
	public void setBuscadorMaterias(BuscadorMaterias buscadorMaterias) {
		this.buscadorMaterias = buscadorMaterias;
	}
