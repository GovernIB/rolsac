...
create table RSC_SIAPDT  (
  ...
  COMMENT ON COLUMN RSC_TRAFIC.TFI_BANNER   	IS 'Banner';
   ...
   --CREAR FKs TRADUCCIONFICHA
    ALTER TABLE RSC_TRAFIC ADD CONSTRAINT RSC_TFIIMA_FK FOREIGN KEY (TFI_IMAGEN) REFERENCES RSC_ARCHIV(ARC_CODI);
    ALTER TABLE RSC_TRAFIC ADD CONSTRAINT RSC_TFIICO_FK FOREIGN KEY (TFI_ICONO)  REFERENCES RSC_ARCHIV(ARC_CODI);
    ALTER TABLE RSC_TRAFIC ADD CONSTRAINT RSC_TFIBAN_FK FOREIGN KEY (TFI_BANNER) REFERENCES RSC_ARCHIV(ARC_CODI);

    --INCLUIR TIPO DE EJECUCION EN SIA.
    ALTER TABLE RSC_SIAJOB ADD SIJ_TIPO VARCHAR2(3);
     COMMENT ON COLUMN RSC_SIAJOB.SIJ_TIPO   	IS 'Tipo de ejecución. PDT (pendiente), TOT (Tots) o TMP (Actualización por timepo)';
