...
alter table RSC_TRALET drop constraint RSC_TRALET_FK;
...
alter table RSC_PROCED drop constraint RSC_PRO_CODLEG_FK;
...
alter table RSC_TRASER drop constraint RSC_TSRARC_FK;
...
alter table RSC_SERVIC drop constraint RSC_SER_CODLEG_FK;
...
alter table RSC_TRAPRO drop constraint RSC_TPRARC_FK;
...
drop table RSC_USUPEG cascade constraints;
drop table RSC_TRANOR cascade constraints;
drop table RSC_BOLETI cascade constraints;
drop table RSC_SILADM cascade constraints;
...
drop table RSC_SERMAT cascade constraints;
drop table RSC_DESTIN cascade constraints;
drop table RSC_IDIOMA cascade constraints;
drop table RSC_SIAJOB cascade constraints;
...
drop table RSC_UNANOR cascade constraints;
...
drop table RSC_PROMSJ cascade constraints;
drop table RSC_SCRKEY cascade constraints;
drop table RSC_SECCIO cascade constraints;
drop table RSC_PEGSEC cascade constraints;
...
drop table RSC_TRAPEG cascade constraints;
drop table RSC_TRAEXD cascade constraints;
drop table RSC_TRAPEC cascade constraints;
drop table RSC_SERMSJ cascade constraints;
...
drop table RSC_POBPRO cascade constraints;
drop table RSC_AGHEVI cascade constraints;
drop table RSC_TRALET cascade constraints;
drop table RSC_MATAGR cascade constraints;
drop table RSC_TRAHEV cascade constraints;
drop table RSC_PERGES cascade constraints;
...
drop table RSC_LOPDLEG cascade constraints;
drop table RSC_TRASEC cascade constraints;
drop table RSC_SERNOR cascade constraints;
...
drop table RSC_DOCNOR cascade constraints;
...
drop table RSC_SERVIC cascade constraints;
drop table RSC_SIAPDT cascade constraints;
...
drop table RSC_SOLJOB cascade constraints;
drop table RSC_SIAUA cascade constraints;
...
drop table RSC_TRADSR cascade constraints;
...
create table RSC_USUPEG (
   USP_CODPEG number(19,0) not null,
   USP_CODUSU number(19,0) not null,
   primary key (USP_CODUSU, USP_CODPEG)
);
create table RSC_TRANOR (
   TNO_CODNOR number(19,0) not null,
   TNO_SECCIO varchar2(256),
   TNO_APARTA varchar2(256),
   TNO_PAGINI number(10,0),
   TNO_PAGFIN number(10,0),
   TNO_TITULO varchar2(1024),
   TNO_ENLACE varchar2(512),
   TNO_RESPON varchar2(512),
   TNO_OBSERV varchar2(4000),
   TNO_CODIDI varchar2(2) not null,
...
create table RSC_SIAJOB (
   ...
   SIJ_DESBRE clob,
   SIJ_DESCRI clob,
   SIJ_ESTADO number(10,0),
   SIJ_TIPO varchar2(255),
   primary key (SIJ_ID)
);
...
create table RSC_SOLRPD (
   SLP_ID number(19,0) not null,
   SLP_TIPO varchar2(255),
   SLP_IDELEM number(19,0),
   SLP_ACCION number(10,0),
   SLP_FECCRE date,
   SLP_FECIDX date,
   SLP_RESULT number(10,0),
   SLP_TXTERR varchar2(300),
   primary key (SLP_ID)
);
...
create table RSC_UNANOR (
   UNN_CODI number(19,0) not null,
   UNN_CODNOR number(19,0),
   UNN_CODUNA number(19,0),
...
create table RSC_TRAFIC (
   ...
   TFI_DESABR clob,
   TFI_DESCRI clob,
   TFI_URLFOR varchar2(255),
   TFI_URLVID varchar2(255),
   TFI_ICONO number(19,0),
   TFI_IMAGEN number(19,0),
   TFI_BANNER number(19,0),
   TFI_CODIDI varchar2(2) not null,
   primary key (TFI_CODFIC, TFI_CODIDI)
);
create table RSC_PROMSJ (
   PMN_CODI number(19,0) not null,
   PMN_USUARIO varchar2(100),
   PMN_USULEC varchar2(100),
   PMN_TEXTO varchar2(256),
   PMN_FECCRE date,
   PMN_FECLEC date,
...
create table RSC_PEGSEC (
   PGS_CODPEG number(19,0) not null,
   PGS_CODSEC number(19,0) not null,
...
create table RSC_TRAPEG (
   TPG_CODPEG number(19,0) not null,
...
create table RSC_SERMSJ (
   SMN_CODI number(19,0) not null,
   SMN_USUARIO varchar2(100),
   SMN_USULEC varchar2(100),
   SMN_TEXTO varchar2(256),
   SMN_FECCRE date,
   SMN_FECLEC date,
   SMN_GESTOR number(1,0),
   SMN_LEIDO number(1,0),
   SMN_SERCODI number(19,0),
   primary key (SMN_CODI)
);
create table RSC_TIPO (
   TIP_CODI number(19,0) not null,
   TIP_CODSIA number(19,0),
   TIP_IDBOIB number(19,0),
...
create table RSC_POBPRO (
   PPR_CODPRO number(19,0) not null,
   PPR_CODPOB number(19,0) not null,
...
create table RSC_TRALET (
   TLE_CODLEG number(19,0) not null,
   TLE_NOMBRE varchar2(1024),
   TLE_CODIDI varchar2(2) not null,
...
create table RSC_PERGES (
   PEG_CODI number(19,0) not null,
   PEG_ORDEN number(10,0),
   PEG_CODEST varchar2(255),
   PEG_DUPLIC varchar2(255),
...
create table RSC_LOPDLEG (
   LEG_CODI number(19,0) not null,
   LEG_IDENTI varchar2(255),
   LEG_DEFAULT number(1,0),
...
create table RSC_SERNOR (
   SRN_CODNOR number(19,0) not null,
   SRN_CODSER number(19,0) not null,
...
create table RSC_TRASER (
   TSR_CODSER number(19,0) not null,
   TSR_NOMBRE varchar2(256),
   TSR_OBJETO varchar2(256),
   TSR_DESTIN clob,
   TSR_REQUIS clob,
   TSR_OBSERV clob,
...
create table RSC_SERVIC (
   SER_CODI number(19,0) not null,
   SER_VALIDA number(10,0),
   SER_CODIGO varchar2(256),
   SER_CODSIA varchar2(12),
   SER_ESTSIA varchar2(1),
   SER_FECSIA date,
   SER_TASURL varchar2(256),
   SER_NOMRSP varchar2(256),
   SER_CORREO varchar2(256),
   SER_TELEFO varchar2(256),
   SER_FECPUB date,
   SER_FECDES date,
   SER_FECACT date,
   ...
   SER_CODLEG number(19,0),
   primary key (SER_CODI)
);
create table RSC_SIAPDT (
   SIP_ID number(19,0) not null,
   SIP_TIPO varchar2(255),
   SIP_IDELEM number(19,0),
   SIP_ESTADO number(10,0),
   SIP_FECALT date,
   SIP_FECIDX date,
   SIP_MENSA varchar2(255),
   SIP_EXISTE number(10,0),
   SIP_SIA number(19,0),
   SIP_SIAUA number(19,0),
   primary key (SIP_ID)
);
...
create table RSC_SIAUA (
   SUA_CODI number(19,0) not null,
   SUA_CODUNA number(19,0),
   SUA_USUARI varchar2(255),
   SUA_CONTRA varchar2(255),
...
create table RSC_POBSER (
   PSR_CODSER number(19,0) not null,
   PSR_CODPOB number(19,0) not null,
...
create table RSC_TRADSR (
   TDS_CODDSR number(19,0) not null,
   TDS_TITULO varchar2(256),
   TDS_DESCRI varchar2(4000),
   TDS_CODARC number(19,0),
   TDS_CODIDI varchar2(2) not null,
...
alter table RSC_TRALET add constraint RSC_TRALET_FK foreign key (TLE_CODLEG) references RSC_LOPDLEG;
...
alter table RSC_PROCED add constraint RSC_PRO_CODLEG_FK foreign key (PRO_CODLEG) references RSC_LOPDLEG;
...
alter table RSC_TRASER add constraint RSC_TSRARC_FK foreign key (TSR_LOPDIA) references RSC_ARCHIV;
...
alter table RSC_SERVIC add constraint RSC_SER_CODLEG_FK foreign key (SER_CODLEG) references RSC_LOPDLEG;
...
alter table RSC_TRAPRO add constraint RSC_TPRARC_FK foreign key (TPR_LOPDIA) references RSC_ARCHIV;
