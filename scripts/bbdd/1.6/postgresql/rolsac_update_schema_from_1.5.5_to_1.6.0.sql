...
(
  ...
  DNO_CODNOR int8 NOT NULL ,
  DNO_ORDEN  int8 NOT NULL ,
  PRIMARY KEY (DNO_CODI),
  CONSTRAINT RSC_DOCNOR_ORDEN_UNIQUE UNIQUE (DNO_CODNOR, DNO_ORDEN) ,
  CONSTRAINT RSC_DOCNOR_NORMAT_FK FOREIGN KEY (DNO_CODNOR) REFERENCES RSC_NORMAT (NOR_CODI) 
);
...
(
    ...
    CONSTRAINT RSC_TRADNR_ARCHIV_FK FOREIGN KEY (TDN_CODARC) REFERENCES RSC_ARCHIV (ARC_CODI) ,
    CONSTRAINT RSC_TRADNR_DOCNOR_FK FOREIGN KEY (TDN_CODDNR) REFERENCES RSC_DOCNOR (DNO_CODI) 
);
...
(
  UNN_CODNOR int8 NOT NULL,
  UNN_CODUNA int8 NOT NULL,
  UNN_CODI   int8 NOT NULL,
  CONSTRAINT RSC_UNN_PK PRIMARY KEY (UNN_CODI),
  CONSTRAINT RSC_UNNUNA_FK FOREIGN KEY (UNN_CODUNA) REFERENCES RSC_UNIADM (UNA_CODI) ,
  CONSTRAINT RSC_UNNNOR_FK FOREIGN KEY (UNN_CODNOR) REFERENCES RSC_NORMAT (NOR_CODI) ,
...
ALTER TABLE RSC_NORMAT ADD NOR_TYPEN VARCHAR(64);
ALTER TABLE RSC_NORMAT ADD NOR_VALIDN int4;
ALTER TABLE RSC_NORMAT ADD NOR_CODBOL_ANT int8;
ALTER TABLE RSC_NORMAT ADD NOR_NUMNOR VARCHAR(255);
ALTER TABLE RSC_NORMAT ALTER COLUMN NOR_TYPE drop not null;
ALTER TABLE RSC_NORMAT ADD NOR_DATVAL bool;
...
CREATE TABLE RSC_TRANOR
  (
   ...
   TNO_CODIDI varchar(2) not null,
    ...
    CONSTRAINT RSC_TNONOR_FK FOREIGN KEY (TNO_CODNOR) REFERENCES RSC_NORMAT (NOR_CODI) ,
    CONSTRAINT RSC_TNOARC_FK FOREIGN KEY (TNO_CODARC) REFERENCES RSC_ARCHIV (ARC_CODI) 
  ) ;
...
(
   ...
   SER_SERRSP int8,
    ...
    CONSTRAINT RSC_SER_INSTRU_FK      FOREIGN KEY (SER_INSTRU)        REFERENCES RSC_UNIADM (UNA_CODI) ,
    CONSTRAINT RSC_SET_SERRSP_FK      FOREIGN KEY (SER_SERRSP)        REFERENCES RSC_UNIADM (UNA_CODI) 
);
...
CREATE TABLE RSC_TRASER (
   ...
   TSR_OBJETO varchar(4000),
   TSR_DESTIN text,
   TSR_REQUIS text,
   TSR_OBSERV text,
   TSR_CODIDI varchar(2) not null,
  CONSTRAINT RSC_TSR_PK PRIMARY KEY (TSR_CODSER, TSR_CODIDI),
  CONSTRAINT RSC_TRASER_SER_FK FOREIGN KEY (TSR_CODSER) REFERENCES RSC_SERVIC (SER_CODI) 
);  
...
(
  ...
  CONSTRAINT RSC_DSRSER_ORDEN_UNIQUE UNIQUE (DSR_CODSER, ORDEN) ,
  CONSTRAINT RSC_DSRSER_NORMAT_FK FOREIGN KEY (DSR_CODSER) REFERENCES RSC_SERVIC (SER_CODI) 
);
...
(
    ...
    TDS_ENLACE varchar(256),
    TDS_DESCRI varchar(4000),
    TDS_CODARC int8,
    TDS_CODIDI varchar(2) not null,
    PRIMARY KEY (TDS_CODDSR, TDS_CODIDI),
    CONSTRAINT RSC_TRADSR_ARCHIV_FK FOREIGN KEY (TDS_CODARC) REFERENCES RSC_ARCHIV (ARC_CODI) ,
    CONSTRAINT RSC_TRADSR_DOCNOR_FK FOREIGN KEY (TDS_CODDSR) REFERENCES RSC_DOCSER (DSR_CODI) 
);
...
CREATE TABLE RSC_HEVISR
  (
    ...
    HVS_CODSER int8,
    HVS_ORDEN int4,
    CONSTRAINT RSC_HVS_PK PRIMARY KEY (HVS_CODI) ,
    CONSTRAINT RSC_HVSHEV_FK FOREIGN KEY (HVS_CODHEV) REFERENCES RSC_HECVIT (HEV_CODI) ,
    CONSTRAINT RSC_HVSSER_FK FOREIGN KEY (HVS_CODSER) REFERENCES RSC_SERVIC (SER_CODI) 
  );
...
--Tabla de historicos
alter table RSC_HISTOR add HIS_CODSER	int8;
...
CREATE TABLE RSC_SERMAT (
    ...
    CONSTRAINT RSC_SRM_PK PRIMARY KEY (SRM_CODSER, SRM_CODMAT) ,
    CONSTRAINT RSC_SRMPRO_FK FOREIGN KEY (SRM_CODSER) REFERENCES RSC_SERVIC (SER_CODI) ,
    CONSTRAINT RSC_SRMMAT_FK FOREIGN KEY (SRM_CODMAT) REFERENCES RSC_MATERI (MAT_CODI) 
);
...
CREATE TABLE RSC_SERNOR (
    ...
    CONSTRAINT RSC_SRN_PK PRIMARY KEY (SRN_CODSER, SRN_CODNOR) ,
    CONSTRAINT RSC_SRNSER_FK FOREIGN KEY (SRN_CODSER) REFERENCES RSC_SERVIC (SER_CODI) ,
    CONSTRAINT RSC_SRNNOR_FK FOREIGN KEY (SRN_CODNOR) REFERENCES RSC_NORMAT (NOR_CODI) 
);
...
(
    ...
    CONSTRAINT RSC_POBSER_PK PRIMARY KEY (PSR_CODSER, PSR_CODPOB) ,
    CONSTRAINT RSC_PSRSER_FK FOREIGN KEY (PSR_CODSER) REFERENCES RSC_SERVIC (SER_CODI) ,
    CONSTRAINT RSC_PSRPOB_FK FOREIGN KEY (PSR_CODPOB) REFERENCES RSC_PUBOBJ (POB_CODI) 
);
...
ALTER TABLE RSC_SOLJOB ADD JOB_FECSER timestamp;
ALTER TABLE RSC_SOLJOB ADD JOB_TOTSER float4;
ALTER TABLE RSC_SOLJOB ADD JOB_TOTDSR float4;
...
ALTER TABLE RSC_TIPO ADD TIP_IDBOIB int8;
...
--Borrado de FK de las tablas de traducciones antiguas
